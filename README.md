...
> 세 번째 방향은 원래 "다관점 적대 리뷰(고정 5렌즈)"였는데, 실사용 기록을 세어보니 **코드 리뷰에서 단독 적발이 0건**이었습니다(같은 회사 모델이라 저와 사각지대를 공유합니다). 반대로 **그 프로젝트가 명문화한 규칙을 어겼는지 감사하는 일**은 외부 모델이 원리적으로 못 하는 것이라, 거기로 역할을 바꿨습니다. 자세한 근거는 `SKILL.md`의 실측 각주에.
